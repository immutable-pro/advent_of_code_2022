...
pub fn part1() {
    ...
    let result = read_file_lines("input/d01.txt").iter().fold(
        Count {
            max_calories: 0,
            prev_calories: 0,
        },
        |mut count, elf_annotation| {
            if elf_annotation.is_empty() {
                if count.prev_calories > count.max_calories {
                    count.max_calories = count.prev_calories;
                }
                count.prev_calories = 0;
                return count;
            }
            
            let calories: u32 = elf_annotation.parse().unwrap();
            count.prev_calories += calories;
            count
        },
    );
